...
class NeuralNetwork:
    ...
    def __init__(self, x, y):
        ...
        self.weights2   = np.random.rand(29, 10)
    ...
    def backprop(self):
        ...
        d_weights1 = np.dot(self.stdize.T, (np.dot((np.dot(2 * (self.y - self.output) * sigmoid_derivative(self.output), self.weights3.T) * sigmoid_derivative(self.layer2)), self.weights2.T) * sigmoid_derivative(self.layer1)))
        ### Split up the final weights
            # print("shapes")
            # print("1  " + str(self.stdize.T.shape))
            # print("12 " + str(self.layer1.T.shape))
            # print("2  " + str((2 * (self.y - self.output).shape)))
            # print("3  " + str(self.weights2.T.shape))
            # print("42   " + str((np.dot(2 * (self.y - self.output) * sigmoid_derivative(self.output), self.weights3.T) * sigmoid_derivative(self.layer2)).shape))
            #
            # print("4  " + str((np.dot(2 * (self.y - self.output) * sigmoid_derivative(self.output), self.weights2.T) * sigmoid_derivative(self.layer1)).shape))
        #print("5  " + str(2 * (self.y - self.output) * sigmoid_derivative(self.output)).shape)
        #d_weights1 = np.dot(self.stdize.T, (np.dot(2 * (self.y - self.output) * sigmoid_derivative(self.output), self.weights2.T) * sigmoid_derivative(self.layer1)))
...
def neural_network_test(nn_2500, nn_2000, nn_1500, nn_1000, ticker):
    ...
    for i in range(0, len(true_close_prices)):
        ...
        nn_2500_output = nn_2500.think(X_close)
        ...
        print("This is output straight from nn " + str(nn_2500_output[0]))
        print("This is denormalized " + str(nn_2500.denormalize_data(nn_2500_output)))
...
def creating_classifying_set(df_col="Close"):
    ...
    for i in range(2500):
        ...
        for i in range(0, len(all_y)):
            print(nn_2500.output[len(all_y)-1, i])
            print(nn_2500.y[i])
...
def candlestick_AI():
    ...
    close_list = neural_network_test(nn_2500, nn_2000, nn_1500, nn_1000, "myx")
